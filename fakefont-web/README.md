...
| `add_thai()`               | `addThai()`                            |
| `add_cjk_basic()`          | `addCJKBasic()`                        |
| `add_tamil()`              | `addTamil()`                           |
| `add_telugu()`             | `addTelugu()`                          |
| `add_kannada()`            | `addKannada()`                         |
...
- The `add*` script bindings are generated by `subfont_bindings!` in
  `src/lib.rs`: a script needs one line in that list, and the Rust method it
  forwards to is the only other thing to name. wasm-bindgen does not camel-case
  method names, so the JavaScript name is given explicitly.
